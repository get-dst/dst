...
_MIGRATE_LOCK_KEY = 0x4B_55_52_4D  # 'KURM' — sibling of the scheduler's 'KURA' lock
...
def _sql(args: argparse.Namespace) -> int:
    ...
    if r.status_code == 404 and args.connection and _declared_connection(args)[0]:
        ...
        print(
            ...
            "this verb runs on the server, which is what puts the probe in the audit log.)",
            file=sys.stderr,
        )
        return 1
    if r.status_code >= 400:
...
def _revoke_token(args: argparse.Namespace) -> int:
    ...
    The verb that did not exist when we needed it. A `dstadm_` token leaked into a
    tracked file (SECRET-AUDIT F1) and there was no way to revoke it: `revoke-key`
    takes a caller name and only touches `api_key`, and admin tokens have no caller.
...
def _test(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "url", None) or getattr(args, "token", None):
        ...
        # is exactly the lie this epic exists to kill.
        print(
...
def _observe(args: argparse.Namespace) -> int:
    ...
    if action == "requests":
        ...
        for r_ in rows:
            ...
            print(
                f"{when}  {str(r_.get('caller') or '-'):<16}{str(r_.get('lens') or '-'):<14}"
                f"{str(r_.get('status') or '-'):<9}{str(r_.get('question') or '')[:60]}"
            )
        return 0
...
def main() -> int:
    ...
    p.add_argument(
        ...
        "belongs to). Caller names are not unique across orgs — this verb never "
