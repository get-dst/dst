...
class ProviderSpec:
    name: str
    type: Literal["anthropic", "openai-compatible", "voyage", "local"]
    api_key: str | None
...
def _spec(name: str, cfg: ProviderConfig) -> ProviderSpec:
    ...
    if cfg.type == "voyage":
        default_embed, default_dim = "voyage-3.5", 1024
    elif cfg.type == "local":
...
_EMBED_SDK = {"voyage": ("voyageai", "voyage"), "local": ("fastembed", "local-embed")}
...
def _embedding_spec() -> ProviderSpec | None:
    ...
    declared resolved to voyage and detonated on its first certified apply,
    ...
    for s in specs().values():
        if not s.embedding_model or s.type not in ("local", "voyage", "openai-compatible"):
            continue
...
def _build_embedder(s: ProviderSpec | None) -> Embedder:
    ...
    if s.type == "voyage":
        ...
        return VoyageEmbedder(s.api_key, model=model, dim=s.embedding_dim)
...
EMBEDDER_HINT = (
    ...
    "(production tier), or a voyage entry (`--extra voyage`)"
)
