...
_COLLAPSED: dict[str, str] = {
    "definition_not_enforceable": (
        "{n} definitions have no sql_expr (prose-only — they guide prose answers "
        "but can't be enforced in generated SQL): {subjects}"
    ),
        ...
        if not (d.sql_expr or "").strip() and d.status != "ambiguous":
            ...
            issues.append(
                Issue(
                    ...
                    message=f"definition '{d.term}' has no sql_expr; it guides prose "
                    "answers but can't be enforced in generated SQL",
                    subject=d.term,
                )
            )
