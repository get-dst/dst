...
# Registration is open by necessity (RFC 7591: the client has no credential yet),
# so the table needs its own retention or it only grows. A registration that was
# never carried into an authorization request expires; one that was is kept
# forever, because a real MCP client re-presents its client_id for the life of the
# integration. The cap is the backstop for a burst inside the TTL — and it, too,
# only ever evicts unused rows, so no amount of registration spam can push a
# working client out of the table.
_CLIENT_TTL = timedelta(days=30)
_MAX_UNUSED_CLIENTS = 500


def _prune_clients(conn: Any) -> None:
    """Drop abandoned registrations: expired first, then oldest-first over the cap."""
    conn.execute(
        text("DELETE FROM oauth_client WHERE last_used_at IS NULL AND created_at < now() - :ttl"),
        {"ttl": _CLIENT_TTL},
    )
    conn.execute(
        text(
            "DELETE FROM oauth_client WHERE client_id IN ("
            "  SELECT client_id FROM oauth_client WHERE last_used_at IS NULL"
            "  ORDER BY created_at DESC OFFSET :cap)"
        ),
        {"cap": _MAX_UNUSED_CLIENTS},
    )
...
def register_client(redirect_uris: list[str], client_name: str | None) -> str:
    ...
    with admin_engine.begin() as conn:
        # Opportunistic, on the endpoint that does the growing — same shape as the
        # `oauth_code_used` sweep above. No scheduler to depend on, and the sweep is
        # paid for by the traffic that makes it necessary.
        _prune_clients(conn)
        conn.execute(
            text(
...
def mark_client_used(client_id: str) -> None:
    """Record that a client_id reached a validated authorization request.

    This is what exempts a row from retention, so it is set at the first real use
    rather than at registration — the whole point is to separate the two.
    """
    with admin_engine.begin() as conn:
        conn.execute(
            text(
                "UPDATE oauth_client SET last_used_at = now() "
                "WHERE client_id = :id AND last_used_at IS NULL"
            ),
            {"id": client_id},
        )
