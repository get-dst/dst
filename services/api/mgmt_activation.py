...
The connect → (context) → audit → lens arc is four surfaces that don't otherwise know
...
def activation(session: Session = Depends(get_app_session)) -> Activation:
    ...
    warehouses = [
        c for c in connection_store.list_connections(session) if c.type not in CONTEXT_SOURCE_TYPES
    ]
