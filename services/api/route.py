...
class _LexicalEmbedder:
    """A deterministic, offline ``Embedder`` for when Voyage is unconfigured.
    ...
    signal is Voyage (the routing eval tunes against it)."""
...
def _route(caller: CallerIdentity, question: str) -> RouteDecision:
    ...
    fast-path), or pure cosine when no LLM is configured. Blocking (Voyage + DeepSeek over
...
async def just_ask(
    ...
    # Off the event loop: routing makes blocking Voyage + DeepSeek calls over sync httpx.
