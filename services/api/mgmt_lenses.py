...
from services.context.ingest import ingest_github, ingest_text
...
@router.get("/{name}/sample-queries")
def get_sample_queries(name: str, session: Session = Depends(get_app_session)) -> dict[str, object]:
    """The draft bundle's 'accepted questions' (semantic_model.sample_queries)."""
    row = store.get_lens(session, name)
    if row is None:
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    draft = row.get("draft")
    ...
    return {
        "queries": [q.model_dump() for q in bundle.semantic_model.sample_queries],
    }


@router.put("/{name}/sample-queries")
def set_sample_queries(
    name: str, body: SampleQueriesBody, session: Session = Depends(get_app_session)
) -> dict[str, int]:
    """Replace the draft bundle's 'accepted questions' and save the draft."""
    row = store.get_lens(session, name)
    if row is None:
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    draft = row.get("draft")
    if draft is None:
        raise HTTPException(status_code=400, detail=f"lens '{name}' has no draft to edit")
    bundle = store.LensBundle.model_validate(draft)
    bundle.semantic_model.sample_queries = list(body.queries)
    if store.update_draft(session, name, bundle) == 0:
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    return {"count": len(body.queries)}
...
@router.post("/{name}/sample-queries/import")
def import_sample_queries(
    name: str, body: SampleQueriesBody, session: Session = Depends(get_app_session)
) -> dict[str, int]:
    """Append accepted questions (e.g. an uploaded eval set) to the draft, deduped.

    Unlike PUT (which replaces), this MERGES: an item is skipped when its question
    already exists (case/space-insensitive) or its sql is empty. The frontend parses
    a pasted/uploaded CSV/JSON into ``queries``.
    """
    row = store.get_lens(session, name)
    if row is None or row.get("draft") is None:
        raise HTTPException(status_code=404, detail=f"lens '{name}' has no draft")
    bundle = store.LensBundle.model_validate(row["draft"])
    existing = {_normalize_q(q.question) for q in bundle.semantic_model.sample_queries}
    added = 0
    for item in body.queries:
        key = _normalize_q(item.question)
        if not item.question.strip() or not item.sql.strip() or key in existing:
            continue
        bundle.semantic_model.sample_queries.append(item)
        existing.add(key)
        added += 1
    if added:
        store.update_draft(session, name, bundle)
    return {"added": added, "total": len(bundle.semantic_model.sample_queries)}


_HARVEST_SYSTEM = (
    "You are given real SQL SELECT statements from a warehouse's query history. For "
    "each, write the SHORT one-line analytics question it answers, in plain English "
    "(don't restate the SQL). Output ONLY a JSON object, no prose, no code fences:\n"
    '{"questions": [{"i": <statement index>, "question": "<one line>"}]}\n'
    "Omit a statement only if it is not a meaningful analytical query."
)


def _draft_questions(statements: list[str]) -> list[SampleQuery]:
    """Ask the assist model to write the question each SQL statement answers."""
    pair = assist_llm()
    if not statements or pair is None:
        return []
    llm, model = pair.llm, pair.name
    listing = "\n".join(f"[{i}] {s}" for i, s in enumerate(statements))
    try:
        data = complete_json(llm, model, _HARVEST_SYSTEM, listing, max_tokens=1500)
    except Exception:  # noqa: BLE001 — bad/empty JSON or provider error: no candidates
        log.exception("question harvest failed")
        return []
    out: list[SampleQuery] = []
    for item in data.get("questions") or []:
        if not isinstance(item, dict):
            continue
        i = item.get("i")
        question = str(item.get("question") or "").strip()
        if isinstance(i, int) and 0 <= i < len(statements) and question:
            out.append(SampleQuery(question=question, sql=statements[i]))
    return out


@router.post("/{name}/sample-queries/harvest")
def harvest_sample_queries(
    name: str,
    days: int = 30,
    limit: int = 15,
    org_id: uuid.UUID = Depends(get_admin_org),
    ...
    """Draft candidate accepted questions from the connection's real query history.

    Pulls the most-run distinct SELECTs (HistoryReader), asks the assist model to
    write the question each answers, and returns them as candidates — NOT saved, so
    the user reviews/edits and keeps the good ones (via import / PUT).
    """
    row = store.get_lens(session, name)
    if row is None or row.get("draft") is None:
        raise HTTPException(status_code=404, detail=f"lens '{name}' has no draft")
    ...
    if not isinstance(connector, HistoryReader):
        raise HTTPException(status_code=400, detail="connection has no query-history catalog")
    statements = [r.statement for r in connector.query_history(days=days, limit=200)]
    candidates = _draft_questions(statements[: max(1, limit)])
    return {"candidates": [c.model_dump() for c in candidates]}


# ─── "use this when…" routing examples ───────────────────────────────────────
...
def _require_draft(session: Session, name: str) -> store.LensBundle:
    row = store.get_lens(session, name)
    if row is None or row.get("draft") is None:
        raise HTTPException(status_code=404, detail=f"lens '{name}' has no draft")
    return store.LensBundle.model_validate(row["draft"])


@router.get("/{name}/use-when")
def get_use_when(name: str, session: Session = Depends(get_app_session)) -> dict[str, object]:
    """The draft's 'use this when…' routing examples."""
    return {"items": list(_require_draft(session, name).semantic_model.use_when)}


@router.put("/{name}/use-when")
def set_use_when(
    name: str, body: UseWhenBody, session: Session = Depends(get_app_session)
) -> dict[str, int]:
    """Replace the draft's routing examples (the human curates what the lens advertises)."""
    bundle = _require_draft(session, name)
    items = list(dict.fromkeys(s.strip() for s in body.items if s.strip()))
    bundle.semantic_model.use_when = items
    store.update_draft(session, name, bundle)
    return {"count": len(items)}


@router.post("/{name}/use-when/generate")
def generate_use_when_endpoint(
    name: str, session: Session = Depends(get_app_session)
) -> dict[str, object]:
    """Seed routing examples from the draft's semantic model and save them to the draft."""
    bundle = _require_draft(session, name)
    pair = assist_llm()
    if pair is None:
        raise HTTPException(status_code=503, detail="no assist model configured")
    llm, model = pair.llm, pair.name
    items = generate_use_when(
        llm,
        model,
        bundle.semantic_model,
        display_name=bundle.config.display_name,
...
@router.post("/{name}/context/github")
def ingest_github_context(
    name: str, body: GithubContextBody, session: Session = Depends(get_app_session)
) -> dict[str, int]:
    """Ingest files from a GitHub repo into the lens's context layer — chunked and
    embedded. `paths` scope the pull; `ref` and `token` are optional."""
    if not store.lens_exists(session, name):
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    embedder = require_embedder()
    # embedding failures (incl. rate limits) raise ProviderError -> global 502 handler
    return ingest_github(
        session, name, body.repo, body.paths, embedder, ref=body.ref, token=body.token
    )


def _require_lens_and_embedder(session: Session, name: str) -> Embedder:
    if not store.lens_exists(session, name):
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    return require_embedder()


@router.get("/{name}/context")
def list_context(name: str, session: Session = Depends(get_app_session)) -> dict[str, object]:
    """The lens's context layer: per-source chunk counts + freshness."""
    if not store.lens_exists(session, name):
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    return {"lens": name, "sources": ctx_store.list_sources(session, name)}


@router.get("/{name}/context/chunks")
def list_context_chunks(
    name: str, source: str | None = None, session: Session = Depends(get_app_session)
) -> dict[str, object]:
    """The actual chunk texts for a lens (optionally one source) — for inspection."""
    if not store.lens_exists(session, name):
        raise HTTPException(status_code=404, detail=f"lens '{name}' not found")
    return {"lens": name, "chunks": ctx_store.list_chunks(session, name, source)}
...
@router.post("/{name}/context/text")
def ingest_text_context(
    name: str, body: TextContextBody, session: Session = Depends(get_app_session)
) -> dict[str, int]:
    """Ingest a raw text blob into the lens's context layer under the given
    source name — chunked and embedded like any other source."""
    embedder = _require_lens_and_embedder(session, name)
    return ingest_text(session, name, body.source, body.text, embedder)


@router.post("/{name}/context/upload")
async def upload_context(
    name: str,
    file: UploadFile = File(...),
    session: Session = Depends(get_app_session),
) -> dict[str, int]:
    """Upload a document — its text is extracted, chunked, and embedded into the
    lens's context. 413 over the `DST_MAX_UPLOAD_MB` cap; 400 when the file
    type can't be read as text."""
    embedder = _require_lens_and_embedder(session, name)
    ...
    while chunk := await file.read(1024 * 1024):
        ...
        if size > limit:
            raise HTTPException(
                ...
                "(raise DST_MAX_UPLOAD_MB to allow)",
            )
        chunks.append(chunk)
    data = b"".join(chunks)
    try:
        body = extract_text(file.filename or "upload", data)
    except UnsupportedFile as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return ingest_text(session, name, f"upload:{file.filename}", body, embedder)


@router.delete("/{name}/context/{source:path}", status_code=204)
def delete_context_source(
    name: str, source: str, session: Session = Depends(get_app_session)
) -> None:
    """Drop one context source and its chunks from the lens. 404 unknown."""
    if ctx_store.delete_source(session, name, source) == 0:
        raise HTTPException(status_code=404, detail=f"context source '{source}' not found")
