...
def quote_table(table: str, dialect: str) -> str:
    """A possibly schema-qualified physical table name.

    Part by part everywhere except BigQuery, where a path with ANY quoted part
    is wrapped whole: `` `proj-with-hyphens.dataset.table` ``, never
    `` `proj-with-hyphens`.dataset.table ``.

    Both are valid BigQuery, and emitting both is what caused the trouble: the
    generation prompt taught the per-part form while the sampling SQL wrapped
    the path whole, so a model shown one and primed on the other closed the path
    it had opened per part — `` `proj`.dataset.table` ``, a stray trailing
    backtick that turns an answerable question into a syntax error, every run.
    One style leaves no half-quoted shape to complete, and this is the one
    BigQuery's own documentation uses.

    MySQL is the other backtick dialect and must NOT do this: there
    `` `db.table` `` names a single table containing a dot, not a qualified
    path. Everything else delimits with double quotes and never had the
    ambiguity."""
    parts = table.split(".")
    quoted = [quote_ident(part, dialect) for part in parts]
    if dialect == "bigquery" and len(parts) > 1 and quoted != parts:
