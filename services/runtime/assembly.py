...
prose context, the certified-definitions page, profile enrichment,
...
def retrieve_context(
    lens: str, qvec: list[float] | None, org_id: uuid.UUID | str
) -> list[ContextChunk]:
    """Best-effort RAG: fetch top-k lens-scoped chunks for a precomputed question vector."""
    if qvec is None:
        return []
    try:
        with org_session(org_id) as session:
            hits = ctx_store.search(session, lens, qvec, k=6)
        return [ContextChunk(text=h.text, source=h.source, score=h.score) for h in hits]
    except Exception as exc:
        log.exception("context retrieval failed for lens %s", lens)
        ctx_store.LAST_SERVING_ERROR["error"] = f"context retrieval failed: {exc}"
        return []
...
def assemble(
    ...
    *,
    ...
    prose = retrieve_context(name, qvec, org_id)
