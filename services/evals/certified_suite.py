...
with fake harnesses. Production callers — the publish gate and
``dst test`` — build the real harness
...
def run_certified_suite(
    ...
    *concurrency* (default settings.gate_concurrency) pools the cases like the
    behavioral runner: provider-bound HTTP, no shared mutable state, results in
    answer order. ``1`` is the exact serial path (the whole test suite pins it —
    the scripted-clock budget tests rely on its one-read-per-iteration
    contract); above 1, each pooled case times itself and checks the deadline
    at its own start."""

    def _score_answer(a: CertifiedAnswer) -> CertifiedCaseResult | None:
        return _score_one(
            a,
            connector=connector,
            lens=lens,
    ...
    if workers > 1:
        active = [a for a in answers if is_active(a.status)]
        if len(active) > 1:

            def _timed(a: CertifiedAnswer) -> CertifiedCaseResult | None:
                start = time.monotonic()
                if deadline is not None and start >= deadline:
                    return None  # not reached — reported by the caller, like serial
                r = _score_answer(a)
                if r is not None:
                    delta = time.monotonic() - start
                    if math.isfinite(delta):
                        r.elapsed_s = round(delta, 3)
                return r

            from concurrent.futures import ThreadPoolExecutor

            with ThreadPoolExecutor(max_workers=min(workers, len(active))) as pool:
                pooled = [r for r in pool.map(_timed, active) if r is not None]
            return CertifiedSuiteOutcome(lens=lens, results=pooled)
...
def _score_one(
    ...
    *,
    ...
    composer: AnswerComposer,
    model_name: str,
) -> CertifiedCaseResult | None:
    """One answer's oracle-vs-generation verdict — the per-case body shared by
    the serial and pooled paths (no clock reads in here: the serial loop's
    one-read-per-iteration contract depends on it)."""
    ...
    for _attempt in range(runner.GATE_ATTEMPTS):
        ...
        if passed:
            if _attempt > 0:
                # Wobble reads as wobble, and the operator
                # learns which questions to certify against it.
                reason = (
                    f"{runner.FLAKY_PREFIX}passed on attempt {_attempt + 1} "
