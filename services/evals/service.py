...
from services.db.session import org_session, set_org_context
...
# eval_run.mode for the scheduler's standing parity suite (plain column, no
# migration; the runner's Literal modes are its own in-memory contract).
STANDING_MODE = "certified"
...
def run_standing_certified(org_id: uuid.UUID | str, cutoff: datetime) -> int:
    """The standing cadence: every published lens with active
    certified answers whose latest '{STANDING_MODE}' run predates *cutoff* re-runs
    the full-corpus parity suite; the outcome persists as a normal eval run, so
    the accuracy trend accrues without benchmark days. Returns runs recorded.
    Never raises: a lens that can't run (no connection, unresolvable model) logs
    and is retried next interval, like the audit scheduler's connections."""
    ...
    with org_session(org_id) as session:
        for name, _dn, _desc, bundle in lens_store.list_published(session):
            latest = session.execute(
                sa_text(
                    "SELECT started_at FROM eval_run WHERE lens = :l AND mode = :m "
                    "ORDER BY started_at DESC LIMIT 1"
                ),
                {"l": name, "m": STANDING_MODE},
            ).first()
            ...
            if not answers:
                continue
            pair = registry.resolve(bundle.config.model.model_ref())
            if pair is None:
                log.info("standing evals: no model resolves for lens %s — skipped", name)
                continue
            ...
            try:
                ...
                outcome = run_certified_suite(
                    ...
                    composer=AnswerComposer(llm, model=model_name),
                    model_name=model_name,
                )
            except Exception:
                log.exception("standing evals failed for lens %s", name)
                continue
            passed = sum(1 for r in outcome.results if r.passed)
            errored = sum(1 for r in outcome.results if "error" in r.oracle_result)
            run_id = store.create_run(
                ...
                score=outcome.score,
                passed=passed,
                failed=len(outcome.results) - passed - errored,
                errored=errored,
            )
            store.record_results(
                ...
                [
                    EvalResult(
                        run_id=run_id,
                        case_id=f"certified:{r.answer_id}",
                        question=r.question,
                        passed=r.passed,
                        grade="pass"
                        if r.passed
                        else ("errored" if "error" in r.oracle_result else "fail"),
                        # The stage tag rides the canonical eval
                        # history, not just one run's JSON: certified-suite
                        # failures are rows-stage by construction (generated
                        # SQL's rows vs the certified rows; no routing, no
                        # prose in the comparison).
                        checks=(
                            {"wrong_at": "rows"}
                            if not r.passed and "error" not in r.oracle_result
                            else {}
                        ),
                        reason=r.reason,
            ...
            session.commit()
            # Same trap as the audit scheduler: the commit ends the SET LOCAL
            # scope, so without re-setting the org context the next lens's
            # eval_run INSERT is org-less (RLS rejects) and its staleness SELECT
            # is blind (retries every cycle).
            set_org_context(session, org_id)
            ran += 1
            log.info(
                "standing evals ran: %s — %d/%d passed (org %s)",
                name,
                passed,
                len(outcome.results),
                org_id,
