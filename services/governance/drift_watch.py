...
- the scheduler: alongside every scheduled audit, diff the connection's
  live catalog against the stored profiles; breaking drift files ONE
  deduplicated review ticket (origin `drift_audit`) naming the affected
  entities, definitions and certified answers — the queue is the push channel.
- the serve-time backstop: a warehouse execution error on a governed
  query runs the same diff immediately, so the FIRST binder error becomes the
  incident instead of the eighty-first user complaint.
