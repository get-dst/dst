...
Built-in indicative list prices, extended/overridden via DST_AI_PRICING (JSON in the
env/.env: {"model": [usd_per_mtok_in, usd_per_mtok_out]}). An unknown model's
cost is None ("unpriced") — never silently billed at some default price; the
trace stores NULL and Observe renders the gap instead of a made-up number.
"""
