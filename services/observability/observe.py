...
def audit_statement(session: Session, days: int = 30) -> dict[str, object]:
    """The Statement — one payload behind the audit page: what was asked, what
    came back, how much of it carried receipts, what it cost, per lens. Built
    windowed with a prior-window comparison so movement is measured, never
    asserted; a delta against an empty prior window is omitted, not invented.
    Governed traffic only (probe rows excluded, like kpis); every figure
    traces to request_log rows.

    One round trip on purpose: the page is skimmed by someone with thirty
    seconds, and a fan of sequential endpoints would make the fast path the
    stale path."""
    governed = "COALESCE(generator_tier, '') <> 'probe'"
    win = "created_at >= NOW() - make_interval(days => :d)"
    prior = (
        "created_at >= NOW() - make_interval(days => :d2) "
        "AND created_at < NOW() - make_interval(days => :d)"
    )

    def _window(where: str) -> dict[str, float]:
        row = session.execute(
            text(
                "SELECT count(*), "
                "count(*) FILTER (WHERE status = 'ok'), "
                "count(*) FILTER (WHERE status = 'clarification'), "
    ...
    series = [
        ...
        for r in session.execute(
            text(
                ...
                f"FROM request_log WHERE {governed} AND {win} GROUP BY 1 ORDER BY 1"
            ),
            {"d": days},
        )
    ]
    ...
    histogram = {
        ...
        for r in session.execute(
            text(
                "SELECT confidence, count(*) FROM request_log "
                f"WHERE status = 'ok' AND {governed} AND {win} GROUP BY confidence"
            ),
            {"d": days},
        )
    }
    ...
    lens_rows = session.execute(
        text(
            ...
            " ORDER BY er.started_at DESC LIMIT 1) AS gate_score "
            "FROM request_log r JOIN lens l ON l.name = r.lens "
            "WHERE COALESCE(r.generator_tier, '') <> 'probe' "
            "AND r.created_at >= NOW() - make_interval(days => :d) "
            "GROUP BY r.lens, owner, l.degraded ORDER BY asked DESC"
        ),
        {"d": days},
    ).all()
    lenses = [
    ...
    answered = int(current["answered"])
    total_cost = float(current["ai_cost_usd"]) + float(current["wh_cost_usd"])
    certified_active = session.execute(
        text("SELECT count(*) FROM certified_answer WHERE status = 'active'")
    ).scalar()
    open_tickets = session.execute(
        text(
            "SELECT count(*) FROM review WHERE origin IN ('serve_error', 'drift_audit') "
            "AND state = 'open'"
        )
    ).scalar()
    ...
    return {
        ...
        "yield_delta_pp": round(yield_pct - prior_yield, 1)
        if yield_pct is not None and prior_yield is not None
        else None,
        ...
        "series": series,
        "lenses": lenses,
        "certified_active": int(certified_active or 0),
        "open_incident_tickets": int(open_tickets or 0),
    }
