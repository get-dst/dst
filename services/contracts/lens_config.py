...
class ModelConfig(Authored):
    ...
    # Run the review judge synchronously on every generated answer and fold
    # its verdict into the verification report. Off by default (latency + cost);
    # for high-stakes lenses. Certified answers never pay for it.
    inline_judge: bool = Field(
        default=False,
        description="run the review judge synchronously on every generated answer "
        "(latency + cost; for high-stakes lenses)",
    )
    # Run the adversarial reviewer on every generated answer — it challenges
    # the answer's assumptions (definition choice, exclusions, grain, time window)
    # rather than scoring a rubric, and folds into the verification report. Off by
    # default (one extra round-trip); certified answers never pay for it.
    adversarial_review: bool = Field(
        default=False,
        description="run the adversarial reviewer on every generated answer — "
        "challenges definition choice, exclusions, grain, time window",
    )
