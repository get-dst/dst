...
model. The flip runs in three resumable phases:
  ...
  B. re-embed — batches of pending rows, one transaction each. A crash resumes
     ...
     when NULLs exist at the target dim. (certified_answer tolerates NULL during a
  ...
  C. flip — restore NOT NULL where the schema demands it (certified_answer stays
...
_TABLES = (("certified_answer", "question"),)
