...
class ProviderConfig(Authored):
    ...
    type: Literal["anthropic", "openai-compatible", "voyage", "local"] = Field(
        description="wire protocol: 'anthropic', 'openai-compatible' (covers OpenAI, "
        "DeepSeek, Ollama, vLLM, Groq, most gateways), 'voyage' (embeddings-only), "
    ...
    embedding_model: str | None = Field(
        default=None,
        description="embedding model this provider serves (voyage type defaults to "
        "voyage-3.5; openai-compatible uses POST {base_url}/embeddings)",
    )
...
class Settings(BaseSettings):
    ...
    # Public base URL (e.g. https://dst.fly.dev) for building review tracking links.
    ...
    # Context-file upload cap, MB (413 above it). The handler reads uploads into
    # memory for extraction — unbounded, one fat PDF OOMs a 1GB instance.
    ...
    # Standing drift audits: the in-process scheduler re-audits each connection
    # this often. 0 disables the loop (trigger audits via the dashboard/API instead;
    # there is no `dst audit` CLI verb).
    audit_interval_hours: float = 24
    # Standing certified evals: each published lens with active
    # certified answers re-runs its parity suite this often, so the accuracy trend
    # accrues without benchmark days. 0 disables (use `dst test` + cron instead).
    eval_interval_hours: float = 24
