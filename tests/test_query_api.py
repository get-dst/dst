...
@needs_db
def test_seeded_context_chunk_reaches_the_generation_prompt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A chunk in the lens's context store must land in the
    generation prompt. This is the CI-time half of the silent-context-free fix —
    /ready's breaker catches runtime failure; this pins the wiring itself
    (assembly → run_query → generator prompt), which no test asserted before."""
    ...
    org, raw = _make_org_token(settings.database_admin_url)
    _seed_lens(org)
    marker = "NORTHWIND-PARITY-MARKER: repeat means number_of_orders > 1."
    embedder = HashEmbedder()
    with org_session(org) as session:
        ctx_store.upsert_chunks(
            session, "customer_value", "note:policy", [marker], embedder.embed([marker])
        )
        session.commit()
    ...
    monkeypatch.setattr(settings, "providers", fake_llm_providers())
    monkeypatch.setattr("services.llm.registry.resolve_embedder", lambda: embedder)
    monkeypatch.setattr(
        "services.runtime.assembly.IntentSQLGenerator",
        lambda llm, model, temperature=0.0: GroundedSQLGenerator(
            llm, model=model, temperature=temperature
    ...
    monkeypatch.setattr(
        "services.llm.anthropic_provider.AnthropicProvider",
        lambda _key, **_: _RecordingLLM([gen_json, "There are 19."]),
    )
    try:
        r = client.post(
            "/v1/lenses/customer_value/query",
        ...
        assert r.status_code == 200, r.text
        assert any(marker in m.content for call in calls for m in call), (
            "the seeded context chunk never reached any generation prompt"
        )
    finally:
        admin = create_engine(settings.database_admin_url)
        with admin.begin() as c:
            for table in ("context_chunk", "lens_version", "lens"):
