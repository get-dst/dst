...
def test_connector_executes_tagged_queries(tmp_path) -> None:
    """The tag is valid SQL everywhere — including inside the row-cap wrapper."""
    import duckdb
...
def test_miner_excludes_dst_self_traffic() -> None:
    ...
    organic = [
        ...
        rec("SELECT SUM(revenue_eur) AS total FROM gold.sales_revenue_monthly", 3),
    ]
    with query_context(purpose="serve", lens="rev"):
        tagged = [rec(tag_sql("SELECT SUM(other_eur) AS total FROM gold.other_revenue"), 40)]
