...
@needs_db
def test_import_sample_queries_merges_and_dedups() -> None:
    # F6: importing an eval set appends to the draft, skipping duplicates (case/space
    # insensitive) and empty-sql rows.
    org, raw = _org_token()
    h = {"Authorization": f"Bearer {raw}"}
    bundle = jaffle_customer_value_bundle().model_dump(mode="json")
    payload = {
        "queries": [
            {"question": "F6 probe — avg orders per customer", "sql": "SELECT avg(n) FROM c"},
            {"question": "  F6 PROBE — Avg Orders Per Customer ", "sql": "SELECT 1"},  # dup
            {"question": "F6 probe — missing sql", "sql": "   "},  # empty sql
        ]
    }
    try:
        assert client.post("/mgmt/lenses", json=bundle, headers=h).status_code == 201
        first = client.post(
            "/mgmt/lenses/customer_value/sample-queries/import", json=payload, headers=h
        )
        assert first.status_code == 200 and first.json()["added"] == 1  # dup + empty skipped
        again = client.post(
            "/mgmt/lenses/customer_value/sample-queries/import", json=payload, headers=h
        )
        assert again.json()["added"] == 0  # already present
    finally:
        _cleanup(org)
...
def test_harvest_drafts_questions_from_sql(monkeypatch: pytest.MonkeyPatch) -> None:
    # F6: the warehouse-harvest helper turns SQL history into question/SQL candidates.
    import services.api.mgmt_lenses as m
    from services.contracts.fakes import ScriptedLLM

    reply = '{"questions": [{"i": 0, "question": "How many orders?"}]}'
    monkeypatch.setattr(
        m, "assist_llm", lambda: ResolvedModel(ScriptedLLM([reply]), "fake", "fake")
    )
    out = m._draft_questions(["SELECT count(*) FROM orders"])
    assert len(out) == 1
    assert out[0].question == "How many orders?"
    assert out[0].sql == "SELECT count(*) FROM orders"
