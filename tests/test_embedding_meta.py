...
@needs_db
def test_guard_runs_on_the_certify_write_path(org: str) -> None:
    """The guard fronts every embed-for-write path; the certified-answer
    embedder (services/certify/generate.py) is the production writer."""
    from services.db import embedding_meta as meta

    with org_session(org) as s:
        meta.guard_write(s, HashEmbedder())
        assert meta.read_meta(s) == ("HashEmbedder", 1024)
    with org_session(org) as s:
        with pytest.raises(meta.EmbeddingMismatchError, match="dst reindex"):
            meta.guard_write(s, _Small())
...
def test_size_empty_columns_round_trip(org: str) -> None:
    ...
    with admin.connect() as c:
        counts = [
            c.execute(text(f"SELECT count(*) FROM {t}")).scalar_one() for t in ("certified_answer",)
        ]
    if any(counts):
        admin.dispose()
