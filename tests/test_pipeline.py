...
def test_figure_gate_falls_back_to_the_data_block_after_two_failures() -> None:
    ...
    # the invented figure never reaches the caller; the data block does
    assert "99" not in res.response.answer
...
def test_fallback_renders_the_rows_not_just_their_count() -> None:
    ...
    assert res.response.composition == "fallback"
    assert "9,999,999" not in res.response.answer  # the invented figure never ships
