...
def test_certified_sql_is_never_reported_as_departing_from_a_definition() -> None:
    """The same SQL that FAILS the check when generated must ABSTAIN when certified.

    A certified answer's SQL was written and approved by a person and serving it
    runs no model, so there is no generation that could have departed. Reporting
    `fail` there put a governance disclaimer into the caller's own answer text
    ("this answer is not governed by it") on a response that simultaneously read
    `certification: certified` and `confidence: verified` — the product arguing
    with itself in front of the caller."""
    kwargs = dict(
        question="how many repeat customers?",
        sql="SELECT count(*) AS n FROM customers",
        answer_text="There are 100 repeat customers.",
    ...
    generated = build_report(**kwargs, certification="none")
    generated_check = generated.check("definition_applied")
    assert generated_check is not None and generated_check.status == "fail"

    certified = build_report(**kwargs, certification="certified")
    certified_check = certified.check("definition_applied")
    assert certified_check is not None, "the check must still be recorded, not dropped"
    assert certified_check.status == "skip", (
        "certified abstains: nothing generated, nothing departed"
    )
    assert certified_check.reason and "human-approved" in certified_check.reason
    # The grade already carved certified out; the check now agrees with it.
    assert certified.grade == "verified"
...
def test_withheld_draft_is_retained_on_the_trace() -> None:
    ...
    # The draft SENTENCE is withheld; the fallback may QUOTE the rejected
    # figure inside its reason (without the token named,
    # a withheld answer can only be diagnosed by guessing at the wording).
    assert "There are 99 customers." not in res.response.answer
    assert "the answer states 99" in res.response.answer  # self-diagnosing
