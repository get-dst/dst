...
@needs_db
def test_drift_ticket_dedup_is_one_per_fingerprint(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Breaking drift → ONE review ticket naming the affected entities/
    definitions/certified answers; the same drift filed again attaches to it
    instead of filing a sibling — the dedup is the (org_id, fingerprint)
    unique index, not a check-before-insert race. (The serve-time backstop in
    services/api/query.py is the caller of this rail.)"""
    from services.governance import drift_watch
    ...
    try:
        for _ in range(2):
            with org_session(org) as session:
                drift_watch.file_ticket(session, _drift_report(), origin="serve_error")
                session.commit()
        with org_session(org) as session:
