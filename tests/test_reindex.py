...
def seeded_org() -> Iterator[str]:
    """A scratch org holding one certified answer at dim 1024, with the meta
    row claimed by HashEmbedder. Teardown reindexes the shared tables back to
    1024 if a test left them retyped, then removes everything."""
    ...
    try:
        ...
        if any(_dim(t) != 1024 for t in ("certified_answer", "router_anchor")):
...
def test_reindex_1024_to_256_and_back(seeded_org: str) -> None:
    ...
    assert run_reindex(HashEmbedder()) == 0
    assert _dim("certified_answer") == 1024
    with admin_engine.connect() as c:
        assert embedding_meta.read_meta(c) == ("HashEmbedder", 1024)
    with org_session(seeded_org) as s:
        chits = certify_store.search(s, "lensR", HashEmbedder().embed([_QUESTION])[0], k=1)
        assert chits and chits[0].score > 0.99
...
def test_reindex_resumes_from_pending_nulls(seeded_org: str) -> None:
    ...
    (The active+embedded check constraint means an interrupted run leaves rows
    demoted to pending_embedding — exactly the state simulated here.)"""
    with admin_engine.begin() as c:
        c.execute(
            text("UPDATE certified_answer SET embedding = NULL, status = 'pending_embedding'")
        )
    ...
    with org_session(seeded_org) as s:
        hits = certify_store.search(s, "lensR", HashEmbedder().embed([_QUESTION])[0], k=1)
        assert hits and hits[0].answer.question == _QUESTION and hits[0].score > 0.99
