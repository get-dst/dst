...
def test_voyage_type_defaults_its_model() -> None:
    spec = ProviderConfig(type="voyage", api_key="k")
    assert spec.embedding_model is None  # config side bare …
    from services.llm.registry import _spec

    assert _spec("voyage", spec).embedding_model == "voyage-3.5"  # … type default fills it
