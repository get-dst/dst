"""Router-signal experiment: does a semantic decider beat cosine floor+margin?

The router's fast path scores a question against each lens's anchor strings by
cosine similarity, with a floor and a margin deciding when that single signal
is trusted. Embedding similarity compresses: it cannot always tell "two lenses
genuinely cover this" from "uncovered noise that scores similarly against
several". This harness measures whether a model-based decider closes that gap,
on a held-out, contamination-guarded labeled set, using the same
RoutingMetrics/evaluate machinery as the routing eval so numbers are
comparable.

Methods compared (each exposes .route(q) -> RouteDecision so evaluate() scores them all):
  cosine  — services.router.Router with the SHIPPED thresholds (the baseline).
  llm     — the configured decider chooses covered/which/none over the candidates.
  hybrid  — trust cosine on a CONFIDENT single match (fast, no model call); else
            ask the decider (the uncertain region, exactly where cosine is weak).
...
class PrecomputedEmbedder:
    """Serve real vectors from a one-shot batch + disk cache (the Router embeds
    one question per route() call). Missing texts are embedded once via the
    install's configured embedder and persisted, so reruns stay offline."""

    dim = 1024
    _CACHE = Path("/tmp/router_experiment_embeddings.json")
    ...
    def __init__(self, texts: list[str]) -> None:
        ...
        if missing:
            from services.llm.registry import resolve_embedder

            live = resolve_embedder()
            if live is None:
                raise SystemExit(
                    "embedding cache incomplete and no embedder is configured — "
                    "declare an embedding provider (or install local-embed) and rerun"
                )
            for t, v in zip(missing, live.embed(missing), strict=True):
                cache[t] = v
...
def main() -> None:
    ...
    # One embed batch over every anchor + question, then route offline from the cache.
