...
s = [svg_open(960, 300)]
cells = [
    (60, "test", ["every approved answer is a", "regression test on your data"], True),
    (355, "deploy", ["through gates that fail loudly,", "atomic and versioned"], False),
    (650, "audit", ["every call priced, signed,", "and on the ledger"], False),
]
for x, title, subs, hot in cells:
    s.append(
        box(x, 60, 250, 96, AMBER if hot else INK, AMBER_W if hot else SURF, 1.6 if hot else 1.1)
    )
    s.append(t(x + 125, 96, title, 17, AMBER if hot else INK, "middle", True))
    for i, sub in enumerate(subs):
        s.append(t(x + 125, 120 + i * 16, sub, 10.5, DIM, "middle"))
for x in (310, 605):
    s.append(line(x + 2, 108, x + 43, 108, INK, "ai", 1.3))
s.append(
    f'<path d="M 775 156 C 775 252, 185 252, 185 156" fill="none" '
    f'stroke="{AMBER}" stroke-width="1.6" marker-end="url(#aa)"/>'
)
s.append(t(480, 240, "every mistake feeds back in as a test", 11.5, AMBER, "middle", True))
write("fig0-lifecycle", s)
...
s = [svg_open(1060, 330)]
steps = [
    (60, "an answer is served", ["with the SQL and", "a receipt attached"], True),
    (310, "someone doubts it", ["they flag it — or dst", "flags itself; a person rules"], False),
    (560, "it becomes a test", ["the fix is saved as an", "approved answer + its test"], False),
    (810, "it gates every deploy", ["the test re-runs on", "every future change"], False),
]
...
s.append(
    t(
        530,
        268,
        "the corrected answer is served from then on — and the AI learns from it",
        11,
        AMBER,
        "middle",
        True,
...
s += [
    ...
    t(714, 102, "✓ static checks — every file", 10.5, INK),
