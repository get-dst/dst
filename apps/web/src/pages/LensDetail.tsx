...
import {
  ...
  type SemanticModelShape,
  type ValidationReport,
} from '../api/lenses'
import {
  useContext,
  useContextChunks,
  type ContextSource as ContextSourceType,
} from '../api/context'
import {
  useCertified,
  useCreateCertified,
...
import { useDrift, useStandards } from '../api/standards'
import {
  useLensRepo,
  useLensFile,
  useLensVersions,
...
export function LensDetail() {
  ...
  const sm = detail.data?.published?.semantic_model ?? detail.data?.draft?.semantic_model
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as Tab)}>
          ...
          <div className="mt-5">
            ...
            <TabPanel value="context">
              {/* Every branch renders compiled state — what apply landed, never an editor. */}
              <ContextTab name={name} bundle={bundle} sm={sm} />
            </TabPanel>
      ...
      <ul className="space-y-1">
        {versions.map((v) => (
          <li key={v.version} className="flex items-center gap-2 text-[12px]">
            ...
            {v.summary && <span className="truncate text-muted">{v.summary}</span>}
          </li>
        ))}
      </ul>
...
// ─── the merged Context view — a tree of everything that grounds the lens ────
function ContextTab({
  name,
  bundle,
  sm,
  ...
  name: string
  bundle: LensBundleShape | null
  sm?: SemanticModelShape
}) {
  return (
    <div className="space-y-3">
      <p className="text-[13px] text-muted">
        Everything that grounds <span className="font-mono text-text">{name}</span> — the schema it
        reads, the meanings it enforces, the prose it draws on, and how it&apos;s probed.
      </p>
      <SchemaBranch sm={sm} />
      <DefinitionsBranch name={name} bundle={bundle} />
      <SourcesBranch name={name} />
      <CertifiedBranch name={name} />
      <UseWhenBranch sm={sm} />
      <SampleQuestionsBranch sm={sm} />
      <HealthCheckBranch name={name} />
    </div>
  )
}

function HealthCheckBranch({ name }: { name: string }) {
  const qc = useQuickcheck(name)
  const result: QuickcheckResult | undefined = qc.data
  const passed = result ? result.results.filter((r) => r.passed).length : 0
  return (
    <TreeBranch
      label="Health check"
      subtitle="run the accepted questions live and judge each answer — nothing is saved"
      defaultOpen={false}
      action={
        <Button
          variant="secondary"
          size="sm"
          onClick={() => qc.mutate(undefined)}
          disabled={qc.isPending}
        >
          {qc.isPending ? 'Checking…' : 'Run'}
        </Button>
      }
    >
      {qc.isError && (
        <p className="text-[12px] text-red">
          {(qc.error as Error)?.message ?? 'Health check failed.'}
        </p>
      )}
      {!result && !qc.isError && (
        <p className="text-[12px] text-muted">
          Runs each accepted question against the live lens and grades the answer with the
          question&apos;s own SQL as the reference. Nothing is persisted.
        </p>
      )}
      {result && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[13px] font-bold text-text tabular-nums">
            ...
            {result.results.map((r, i) => (
              <li key={i} className="flex items-start gap-2.5 px-3 py-2">
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-[12px] leading-snug text-text">{r.question}</span>
                  {r.reason && (
...
function CertifiedBranch({ name }: { name: string }) {
  const certified = useCertified(name)
  const create = useCreateCertified(name)
  const del = useDeleteCertified(name)
  const qc = useQueryClient()
  const [question, setQuestion] = useState('')
  const [sql, setSql] = useState('')
  const refresh = () => qc.invalidateQueries({ queryKey: ['certified', name] })
  const rows = certified.data ?? []

  return (
    <TreeBranch
      label="Certified answers"
      count={rows.length}
      subtitle="approved question→SQL served directly on a match"
      defaultOpen={false}
    >
      {certified.isLoading && <Skeleton className="h-10 w-full" />}
      <div className="space-y-2">
        {rows.map((c) => (
          <div key={c.id} className="rounded-md border border-border bg-surface px-3 py-2">
            <div className="flex items-start justify-between gap-3">
              <span className="text-[13px] font-medium text-text">{c.question}</span>
              <button
                onClick={() => del.mutate(c.id, { onSuccess: refresh })}
                className="shrink-0 text-[11px] text-muted hover:text-red transition-colors"
                style={{ transitionDuration: 'var(--duration-fast)' }}
              >
                Remove
              </button>
        ...
        {certified.data && rows.length === 0 && (
          <p className="text-[13px] text-muted">
            None yet — rule on a flagged answer under Reviews and certify it, or add one below.
          </p>
        )}
      </div>
      ...
      <div className="mt-3 space-y-2">
        <Input
          ...
          placeholder="question, e.g. how many repeat customers?"
        />
        <textarea
          value={sql}
          onChange={(e) => setSql(e.target.value)}
          placeholder="approved SQL…"
          rows={2}
          className={[
            'w-full rounded-md border border-border bg-bg px-3 py-2 font-mono text-[12px]',
            'text-text placeholder:text-muted-2 transition-colors outline-none resize-y',
        ...
        <Button
          variant="secondary"
          size="sm"
          disabled={!question.trim() || !sql.trim() || create.isPending}
          onClick={() =>
            create.mutate(
              { question: question.trim(), sql: sql.trim() },
              {
                onSuccess: () => {
                  setQuestion('')
                  setSql('')
        ...
        >
          {create.isPending ? 'Adding…' : 'Add certified answer'}
        </Button>
        {create.isError && <p className="text-[13px] text-red">{String(create.error)}</p>}
      </div>
    </TreeBranch>
  )
}

function SchemaBranch({ sm }: { sm?: SemanticModelShape }) {
  if (!sm || sm.entities.length === 0) {
    return (
      <TreeBranch label="Schema" subtitle="tables & fields the lens can read" defaultOpen={false}>
        <p className="text-[13px] text-muted">No semantic model yet.</p>
      </TreeBranch>
    )
  }
  return (
    <TreeBranch
      label="Schema"
      count={sm.entities.length}
      subtitle="tables & fields the lens can read"
      defaultOpen={false}
    >
      <div className="space-y-3">
        {sm.entities.map((e) => (
          <div key={e.name}>
            <div className="flex items-baseline gap-2">
              <span className="font-mono text-[12px] font-bold text-text">{e.name}</span>
              <span className="font-mono text-[11px] text-muted">{e.source.table}</span>
              <span className="text-[11px] text-muted tabular-nums">· {e.fields.length} fields</span>
            </div>
            <div className="mt-1.5 flex flex-wrap gap-1">
              {e.fields.map((f) => (
                <span
                  key={f.name}
                  className="inline-flex items-center gap-1 rounded border border-border bg-surface px-1.5 py-0.5"
                >
                  <span className="font-mono text-[11px] font-medium text-text">{f.name}</span>
...
function UseWhenBranch({ sm }: { sm?: SemanticModelShape }) {
  const items = sm?.use_when ?? []
  return (
    <TreeBranch
      label="Use this when…"
      count={items.length || undefined}
      subtitle="how callers ask for this lens — the router matches a question against these"
      defaultOpen={items.length === 0}
    >
      {items.length === 0 ? (
        <p className="text-[12px] text-muted">
          Example asks that route a question to this lens — authored in the lens&apos;s files
          (<code className="font-mono text-[11px] text-text">use_when</code>); the data team
          curates what it advertises.
        </p>
      ) : (
...
function SampleQuestionsBranch({ sm }: { sm?: SemanticModelShape }) {
  const qs = sm?.sample_queries ?? []
  if (qs.length === 0) return null
  return (
    <TreeBranch
      label="Sample questions"
      count={qs.length}
      subtitle="what callers can ask — authored in queries.yaml"
      defaultOpen={false}
    >
      <ul className="space-y-1.5">
        {qs.map((q, i) => (
...
// Sources, read-only: the prose grounding this lens — whatever was ingested via
// the lens context endpoints (bring your own context: your driver agent or
// scripts POST text/files/repo paths). This branch only renders what landed.
function SourcesBranch({ name }: { name: string }) {
  const ctx = useContext(name)

  return (
    <TreeBranch
      label="Sources"
      count={ctx.data?.sources.length}
      subtitle="the prose that grounds answers"
    >
      <div className="space-y-4">
        {/* Indexed sources */}
        <div className="space-y-2">
          ...
          {ctx.data?.sources.map((s) => (
            <ContextSource key={s.source} name={name} source={s} />
          ))}
          {ctx.data && ctx.data.sources.length === 0 && (
            <p className="text-[13px] text-muted">
              Nothing indexed for this lens yet. Ingest prose from your own tooling:{' '}
              <code className="font-mono text-[12px] text-text">
                POST /mgmt/lenses/{name}/context/text
              </code>{' '}
              for notes, <code className="font-mono text-[12px] text-text">…/context/upload</code>{' '}
              for documents, or{' '}
              <code className="font-mono text-[12px] text-text">…/context/github</code> for repo
              files.
            </p>
          )}
        </div>
...
// Definitions, read-only: the meanings the lens enforces are authored in the
// semantic files (dst apply) — this branch renders the compiled state plus
// any drift against declared standards.
function DefinitionsBranch({ name, bundle }: { name: string; bundle: LensBundleShape | null }) {
  const drift = useDrift(name)
  const defs = bundle?.semantic_model.definitions ?? []
  const conflictFor = (term: string) =>
    drift.data?.conflicts.find((c) => c.term === term) ?? null

  return (
    <TreeBranch
      label="Definitions"
      count={defs.length}
      subtitle="the meanings this lens enforces — authored in files"
    >
      <div className="space-y-4">
        <div className="space-y-2">
          {defs.map((d, i) => {
            const conflict = conflictFor(d.term)
            return (
              <div key={d.term + i} className="rounded-md border border-border bg-surface px-3 py-2.5">
                <span className="font-mono text-[13px] font-bold text-text">{d.term}</span>
                <p className="mt-1 text-[13px] leading-relaxed text-text">{d.body}</p>
                {d.sql_expr && (
                  <pre className="mt-1.5 overflow-x-auto rounded border border-border bg-bg px-2.5 py-1.5 font-mono text-[11px] leading-relaxed text-muted">
                    {d.sql_expr}
                  </pre>
                )}
                {conflict && (
                  <div className="mt-2 rounded-md border border-accent/30 bg-accent-fg px-3 py-2.5 text-[12px] leading-relaxed">
                    <span className="font-medium text-accent-dark">Differs from {conflict.source}: </span>
                    <span className="text-accent-dark">"{conflict.other_body}"</span>
                  </div>
                )}
              </div>
            )
          ...
          {defs.length === 0 && (
            <p className="text-[13px] text-muted">
              No definitions yet — author them in the lens&apos;s semantic files and{' '}
              <code className="font-mono text-[12px] text-text">dst apply</code>.
            </p>
          )}
        </div>
...
// Org standards, read-only — the declared canonical definitions (drift baseline)
// live in the semantic files; this card just shows what governs the comparison.
function OrgStandards() {
  ...
  return (
    <Card>
      <CardHeader>
        <div>
          <span className="text-[13px] font-medium text-text">Org standards</span>
          <p className="mt-0.5 text-[11px] text-muted">Canonical org-wide definitions — the drift baseline, declared in files.</p>
        </div>
      </CardHeader>
      <CardBody className="space-y-3">
        ...
        {standards.data && standards.data.length > 0 && (
          <div className="divide-y divide-border rounded-md border border-border overflow-hidden">
            {standards.data.map((s) => (
              <div key={s.term} className="px-3 py-2 bg-surface">
                <span className="text-[13px]">
                  <span className="font-mono font-medium text-text">{s.term}</span>
        ...
        {standards.data && standards.data.length === 0 && (
          <p className="text-[13px] text-muted">No org standards declared yet.</p>
        )}
      </CardBody>
    </Card>
  )
...
function ContextSource({ name, source }: { name: string; source: ContextSourceType }) {
  const [open, setOpen] = useState(false)
  const chunks = useContextChunks(name, open ? source.source : null)

  const sourceTypeIcon = source.source.startsWith('github:')
    ? (
      <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className="text-muted">
    ...
    : source.source.startsWith('upload:')
    ? (
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none" aria-hidden="true" className="text-muted">
        <rect x="2" y="1" width="9" height="13" rx="1" stroke="currentColor" strokeWidth="1.25" />
        <path d="M9 1v4h4" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" />
        <path d="M13 5L9 1" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" />
      </svg>
    )
    : (
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none" aria-hidden="true" className="text-muted">
  ...
  return (
    <div className="rounded-md border border-border bg-surface overflow-hidden" style={{ boxShadow: 'var(--shadow-card)' }}>
      <div className="flex items-center justify-between px-3 py-2.5">
        <button
          onClick={() => setOpen((o) => !o)}
          className="flex items-center gap-2.5 text-left min-w-0 cursor-pointer rounded hover:text-accent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
          style={{ transitionDuration: 'var(--duration-fast)' }}
        >
          <svg
            width="12"
          ...
          <span className="min-w-0">
            <span className="font-mono text-[12px] text-text truncate">{source.source}</span>
            <span className="ml-2 text-[11px] text-muted tabular-nums">
              {source.chunks} chunk{source.chunks !== 1 ? 's' : ''}
              {source.last_indexed
                ? ` · ${new Date(source.last_indexed).toLocaleDateString()}`
                : ''}
            </span>
          </span>
        </button>
      ...
      {open && (
        <div className="border-t border-border bg-bg px-3 py-3 space-y-2">
          ...
          {chunks.data?.chunks.map((c, i) => (
            <div key={c.id} className="rounded border border-border bg-surface px-3 py-2.5">
              <div className="mb-1 text-[10px] font-medium uppercase tracking-wider text-muted">
                chunk {i + 1}
              </div>
              <p className="whitespace-pre-wrap text-[12px] leading-relaxed text-text">{c.text}</p>
            </div>
          ))}
        </div>
      )}
