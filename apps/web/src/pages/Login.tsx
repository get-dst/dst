...
// The minted dstsess_ token lands in the same localStorage slot the dev
// admin-token fallback uses, so everything downstream is auth-agnostic.
...
export function Login() {
  ...
  const enter = (token: string) => {
    setToken(token.trim())
    // Hard-reload so the whole app queries under the fresh credential.
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null)
    if (tokenMode) {
      if (tokenVal.trim()) enter(tokenVal)
      return
    }
    setPending(true)
    try {
    ...
    <div className="flex h-full items-center justify-center px-4">
      <div className="w-full max-w-[360px]">
        <div className="mb-6 flex items-center justify-center gap-2.5">
          <DstMark />
          <span
            className="select-none text-[17px] font-semibold text-text"
