...
/** Org switcher: lists the orgs this browser has held tokens for. */
  ...
  return (
    <div className="px-3 py-3">
      ...
      <div className="space-y-0.5">
        ...
        {hasToken && !activeName && orgs.length === 0 && (
          <div className="rounded-md bg-surface-2 px-2 py-1.5 text-[12px] text-text">Current org</div>
        )}
      </div>
    </div>
  )
            ...
            <>
              ...
              <div className="px-3 py-2">
                <button
                  type="button"
                  onClick={() => {
                    // Revoke the local session server-side, then back to the login form.
                    void logout().finally(() => {
                      setToken('')
                      window.location.assign('/')
                    })
