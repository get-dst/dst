...
keep the prompt small is the [lens's](lens.md) scope, not truncation.

The user turn carries the per-question context. The certified-definition page
comes **first**. Then the answer contract, **last**, right next to the
...
It does **not** see the semantic model or the certified pages, and no
...
This command renders everything above for a real question, fully assembled — no
