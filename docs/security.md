...
the product. Two optional integrations reach further — Clerk and OIDC
discovery — and each is inert until you configure it; they carry identity,
never warehouse rows. Both are named below.
...
above that is it sampled (`services/contracts/profile.py:26,44`). On BigQuery a
...
The grounding context is file-authored — the semantic model, the governed
definitions, and the certified-definition pages — and its embeddings are
computed at publish, so that text goes to the configured embedding provider
when `dst apply` runs, not per question (`services/router/anchor_store.py`).
...
(`services/context/local_embedder.py`), and two integrations that are inert
until you configure them — OIDC discovery against your IdP
(`services/auth/oidc.py`), and Clerk.

Clerk and the `/docs` API reference are the two exceptions to the no-CDN
rule (the second is detailed under the served surface below). Clerk is opt-in: set a Clerk
...
`clerk-js` from Clerk's CDN (`services/api/oauth.py:324`). Leave it unset — the
default — and no dst page except the `/docs` API reference loads a script from a host you do not run.
...
One port serves three things — the dashboard, the control plane, and the MCP
door — so what that port sends back, and how it holds a credential, is part of
the boundary.

**Session handling.** `POST /auth/login` returns the `dstsess_` token in the
response body and sets it as an httpOnly cookie
(`services/api/auth_local.py`). API clients use the body token; the dashboard
uses the cookie and never writes the session to `localStorage`
(`apps/web/src/pages/Login.tsx`), so script running on the origin cannot read
it out. The cookie is `SameSite=Lax`, and `Secure` whenever the request is
HTTPS or `DST_ENVIRONMENT=production` — the scheme a TLS-terminating proxy
reports is not trusted to decide that. A hand-pasted `dstadm_` admin token has
nowhere else to live and does go to `localStorage`; the dashboard labels it as
the credential in play, because a bearer header outranks the session cookie
server-side. Signing out revokes the session on the server and clears every
credential the browser holds, including tokens for other orgs.

**Login.** Passwords are scrypt with a per-user salt. Verification runs whether
or not the address matches an account, so response time does not reveal which
addresses are real (`services/auth/local.py`), and the number of hashes in
flight is capped so a burst of logins cannot exhaust memory. Attempts are
budgeted per email address and per source address; `POST /oauth/register` — the
one endpoint that must accept anonymous writes, because RFC 7591 clients
register before they have any credential — is budgeted per source address,
bounded in what one registration may store, and swept: a registration that
never reaches an authorization request expires, while a client that has
connected is kept (`services/auth/oauth.py`).

Be clear about the limit of those budgets: they are counted **in process**. They
apply per replica rather than across a fleet, and a restart forgets them. They
raise the cost of guessing and of spraying registrations; they are not a
distributed lockout, and they are not a substitute for a password policy or for
putting the deployment behind whatever rate limiting your edge already does.
...
HSTS is added on HTTPS and in production
(`services/api/security_headers.py`). The dashboard's policy keeps scripts on
`'self'` — the bundle loads no inline script and no third-party script — while
allowing inline styles, which the interface uses throughout. Framing is denied
by both the header and `frame-ancestors`, so the dashboard cannot be embedded
and click-jacked. Two pages state their own policy because they legitimately
load something else: the OpenAPI reference at `/docs`, which loads Swagger UI
from a CDN, and the MCP consent page **when Clerk is configured**, which loads
Clerk's SDK — that page restricts framing but does not restrict what it loads.
