...
older skill on purpose is legitimate, and this command's 1 means *not callable*, not
*not current*.
...
that fails is named individually and the command exits
...
applied (this command runs server-side; `introspect` reads dst.yaml directly and works
...
repeat across tenants, so this command refuses to act rather than guess.
...
history) and asks; `--yes` for headless. File absence never deletes: this command is the
only way.
...
lenses are checked first and the command refuses while any still reads through the
...
is listed here — `dst <command> --help` is the exhaustive one.)*
