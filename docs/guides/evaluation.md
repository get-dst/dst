...
suite runs on demand (`dst test`), as a gate inside `dst apply`, and on a standing
schedule.
...
depends on — re-stamp to the current versions. The standing scheduled run
records the trend but re-stamps nothing.
...
to what a push touches; this verb is the cron/CI sweep). Exit `1` on any
...
![dst test with a certified corpus: three green PASS rows, 3/3 passed](../assets/term/test2.svg) A standing scheduler re-runs the same suite
per published lens every `DST_EVAL_INTERVAL_HOURS` (default 24, `0` disables),
so the accuracy trend accrues without setting aside benchmark days.
