...
Resolution order, for every remote verb (`services/cli/main.py:1540`):
...
Tokens follow the same path: `--token` / `DST_ADMIN_TOKEN` for admin verbs,
`--key` / `DST_API_KEY` for caller verbs like `dst query`. The `--dir` hop is the
...
The project directory is the source of truth, so promotion is not a verb: it is
**the same git ref applied to the next server**. Staging applied commit `abc123`
...
**On the PR — no server writes.** `dst validate` parses and checks every managed
file locally. `dst plan` is the dry run against the target server: it validates
every `semantic/**` file through the same seam apply parses with, so a file apply
would reject **exits 1 here**, on the PR, instead of at deploy time. A clean plan
exits 0.
...
jobs:
  check:
    ...
    steps:
      ...
      - run: pipx install dst
      - run: dst validate --dir .
...
check job points at staging. If you would rather not expose a server to PR
builds, drop to `dst validate` alone: it needs no server, and it catches the file
errors that cause most rejected applies.

## Exit codes

Every verb carries its outcome in the exit code, so a pipeline branches without
parsing prose.

| Verb | 0 | non-zero |
|---|---|---|
| `dst validate` | every check passed | `1` something failed |
